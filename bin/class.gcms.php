...
// เวลาปัจจุบัน
$mmktime = mktime(date("H") + (isset($config['hour']) ? $config['hour'] : 0));
...
class gcms
{
    ...
    public static function CheckLogin($email, $password)
    {
        ...
        if ($email == '') {
            ...
            $userupdate = false;
            $login_result = false;
            if (empty($config['member_login_phone'])) {
    ...
    {
        return isset($_SESSION['login']['status']) && (int) $_SESSION['login']['status'] == 1;
    }
     ...
     * @return mixed
     */
    public static function getLng($matches = array())
    {
        global $lng;

        return isset($lng[$matches[1]]) ? $lng[$matches[1]] : '';
    }
    ...
    public static function ser2Str($datas)
    {
        if ($datas == '') {
            ...
            return isset($datas[LANGUAGE]) ? $datas[LANGUAGE] : $datas[''];
        }
    }
