...
// เรียกใช้งาน ftp
$ftp = new ftp(
    isset($config['ftp_host']) ? $config['ftp_host'] : 'localhost',
    isset($config['ftp_username']) ? $config['ftp_username'] : '',
    isset($config['ftp_password']) ? $config['ftp_password'] : '',
    isset($config['ftp_root']) ? $config['ftp_root'] : '',
    $document_root,
    isset($config['ftp_port']) ? $config['ftp_port'] : 21
);
if (!empty($config['skin']) && $config['db_username'] != '' && $config['db_name'] != '') {
...
define('SKIN', 'skin/'.(isset($config['skin']) ? $config['skin'].'/' : ''));
