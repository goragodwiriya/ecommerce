...
if (is_file('./install/index.php')) {
    ...
    // font size
    $fontSize = isset($_COOKIE['fontSize']) ? (int) $_COOKIE['fontSize'] : 12;
    ...
    if ($action == 'recover') {
        ...
        $login_remember = isset($_POST['remember']) ? (int) $_POST['remember'] : 0;
        if ($login_email == '') {
            $message_type = 'error';
            $message = "$lng[LNG_PLEASE_FILL] $lng[LNG_EMAIL]";
        ...
        $login_email = gcms::decode(isset($_COOKIE[PREFIX.'_login_email']) ? $_COOKIE[PREFIX.'_login_email'] : '');
        $login_password = gcms::decode(isset($_COOKIE[PREFIX.'_login_password']) ? $_COOKIE[PREFIX.'_login_password'] : '');
    ...
    $replace[] = empty($message_type) ? 'hidden' : $message_type;
    $replace[] = isset($message) ? $message : '';
    ...
    $replace[] = DATA_URL;
    $replace[] = isset($_GET['src']) ? $_GET['src'] : '';
