...
if (defined('MAIN_INIT')) {
    ...
    foreach ($db->customQuery($sql) as $item) {
        if (isset($my_quantity) && $my_quantity > 0 && $order_id == $item['id']) {
            // ปรับปรุงจำนวนสินค้าที่เลือก
            $quantity = $my_quantity;
        } else {
            $quantity = $item['quantity'];
            if (isset($product_id) && $product_id == $item['product_id'] && $additional_id == $item['additional_id']) {
