...
if (gcms::isReferer()) {
    ...
    if ($action == 'removecart' && $order_id > 0) {
        ...
        if (sizeof($index) == 1) {
            ...
            $my_quantity = isset($_POST['value']) ? max(1, (int) $_POST['value']) : 1;
            // ตรวจสอบตะกร้าสินค้า
            include ROOT_PATH.'modules/product/loadbasket.php';
            foreach ($basket as $item) {
                if ($order_id == $item['id']) {
                    $ret['cart_price_'.$order_id] = rawurlencode(gcms::int2Curr($my_quantity * $item['net']));
