...
if (defined('MAIN_INIT')) {
    ...
    if (sizeof($index) == 0) {
        ...
        $index = $index[0];
        // เรียงลำดับ
        $sorts = array('`id` DESC', '`topic` ASC', '`topic` DESC', '`price` DESC,`id` ASC', '`price` ASC,`id` ASC', '`visited` DESC,`id` ASC');
        $orders = array();
        foreach ($lng['PRODUCT_SORT'] as $k => $b) {
            $orders[] = array($b, $sorts[$k]);
        }
        // รายการเรียงลำดับ
        ...
        if ($index['module'] != $module_list[0]) {
            ...
            $menu_text = isset($install_modules[$index['module']]['menu_text']) ? $install_modules[$index['module']]['menu_text'] : '';
            $breadcrumbs['MODULE'] = gcms::breadcrumb(
                ...
                ($menu_text == '' ? $index['topic'] : $menu_text),
                $breadcrumb
            );
        }
        ...
        $splitpage = empty($splitpage) ? '<strong>1</strong>' : $splitpage;
        // แสดงผล list รายการ
