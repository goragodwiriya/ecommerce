...
if (defined('MAIN_INIT')) {
    ...
    $a = isset($_REQUEST['a']) ? (int) $_REQUEST['a'] : 0;
    $search = isset($_REQUEST['q']) ? preg_replace('/[+\s]+/u', ' ', $_REQUEST['q']) : '';
    // query ข้อมูล
    ...
    if ((int) $index['id'] == 0) {
        ...
        // โมดูล
        if ($index['module'] != $module_list[0] && isset($install_modules[$index['module']])) {
            $m = isset($install_modules[$index['module']]['menu_text']) ? $install_modules[$index['module']]['menu_text'] : '';
            $breadcrumbs['MODULE'] = gcms::breadcrumb(
                '',
                gcms::getURL($index['module']),
                isset($install_modules[$index['module']]['menu_tooltip']) ? $install_modules[$index['module']]['menu_tooltip'] : '',
                ($m == '' ? $index['module'] : $m),
                $breadcrumb
            );
        }
        ...
        $replace[] = empty($moderator) ? '{LNG_SEND_DELETE}' : '{LNG_DELETE}';
        $replace[] = $index['id'];
        ...
        $replace[] = isset($index['category_id']) ? $index['category_id'] : 0;
    ...
    // เลือกเมนู
    $menu = isset($install_modules[$index['module']]['alias']) ? $install_modules[$index['module']]['alias'] : '';
